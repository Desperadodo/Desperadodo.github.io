...
def xml2mask(img_name, data_root, patch_size = 2560):
    ...
    for i in range(1, w // patch_size - 1):
        for j in range(1, h // patch_size - 1):
            ...
            if mask_patch.mean(axis=0).mean(axis=0) > 20.4:  # 8% * 255
                cv2.imwrite(data_root + "\\mask-neg\\" + img_name + '-' + str(i) + '-' + str(j)+'.jpg', mask_patch)
                ...
                save_file(patch, data_root + "\\data-neg\\" + img_name + '-' + str(i) + '-' + str(j))
            else:
                continue
...
def main():
    data_root = r'I:\Puzzle_Tuning_Datasets\Raw\PAIP2019\Training_phase_1'

    make_and_clear_path(os.path.join(data_root, 'mask'))
    make_and_clear_path(os.path.join(data_root, 'data'))

    all_Annotations = find_all_files(data_root, 'xml')
    for Annotation in all_Annotations:
        img_name = os.path.split(Annotation)[1].split('.')[0]
        # xml2mask(img_name, data_root)
        xml2mask_check(img_name, data_root)

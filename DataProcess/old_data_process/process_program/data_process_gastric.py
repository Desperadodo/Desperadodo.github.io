...
def pc_to_stander(root_from, root_to):
    ...
    with open(r'E:\A_bunch_of_data\Raw\gastric_image\train_label.csv', 'r') as f:
        reader = csv.reader(f)
        result = list(reader)
        length = len(result)
    ...
    pd.DataFrame.from_dict(name_dict, orient='index', columns=['origin path']).to_csv(
        os.path.join(root_target, 'name_dict_gastric.csv')
    )


if __name__ == '__main__':
    """pc_to_stander(root_from=r'E:\A_bunch_of_data\Raw\gastric_image\train_org_image',
                  root_to=r"E:\A_bunch_of_data\Processed\gastric_jpg")"""
